...
get_currentuserinfo();
		...
		{
			$user_ID = $user_login;						//Get the current user ID
...
?>
        <tr valign="top">
          ...
          <td><input name="author_curr" id="author_curr" type="file">
            <br>
            <?php
	if(file_exists($dir.$user_login.'.jpg'))
	{
		$qq=get_option( 'siteurl' ). '/wp-content/uploads/author image/'.$user_login.'.jpg';
...
if(current_user_can("administrator"))
{
	if(!file_exists($dir.$user_login.'.jpg') and !file_exists($dir.'author_default.jpg'))
		echo'disabled';
}
else
	if(!file_exists($dir.$user_login.'.jpg'))
...
if ( !function_exists( 'get_avatar' ) ) :
	function get_avatar( $id_or_email="1", $size = '85px', $default = '', $alt = 'Author Image' ) 
	{
		...
		if ($op)
			$avatar = "<img alt='{$alt}' title='Author Image' src='{$op}' class='avatar avatar-{$size} photo avatar-default'    style='width:{$size}px; height:{$size}px' />";
		else
			$avatar="<img alt='{$alt}' title='Author Image' src='{plugin_dir_path(__FILE__)}default-user-image.png' class='avatar avatar-{$size} photo avatar-default'  style='width:{$size}px; height:{$size}px' />";
